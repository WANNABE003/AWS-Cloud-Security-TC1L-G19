...
The STRIDE model is used to classify possible threats against the SecureStyle fashion e-commerce system. Each category is mapped to a realistic attack scenario and the security controls implemented in the application and SQL database.

| STRIDE Category | Meaning | Example Risk in SecureStyle | Implemented Control |
| --- | --- | --- | --- |
| Spoofing | Pretending to be another user | Attacker uses stolen credentials to log in as an admin or inventory officer | bcrypt password hashing, signed HTTP-only session cookie, login rate limiting |
| Tampering | Unauthorized modification of data | Customer manipulates API request to change product price, stock quantity, or order amount | Server-side validation, parameterized SQL, SQL CHECK constraints, role-based access control |
| Repudiation | Denying that an action happened | Admin or inventory officer denies deleting or updating a fashion product | `AuditLog` table, audit API records, SQL triggers for product and order changes |
| Information Disclosure | Exposing confidential data | Staff views full customer phone number, address, or shipping information unnecessarily | Dynamic data masking, restricted SQL views, least privilege SQL roles |
| Denial of Service | Making the system unavailable | Attacker sends repeated login attempts or oversized API requests | Login rate limiting, JSON request size limit, indexed database queries |
| Elevation of Privilege | Gaining higher access than allowed | Customer calls admin-only delete product API or tries to view audit logs | `requireAuth(["Admin"])`, route-level RBAC, SQL `GRANT` and `DENY` permissions |

### STRIDE Category Summary

| Category | Affected Asset | Security Objective |
| --- | --- | --- |
| Spoofing | User accounts and sessions | Authentication |
| Tampering | Product, stock, and order records | Integrity |
| Repudiation | Administrative actions and product changes | Accountability |
| Information Disclosure | Customer personal data and order details | Confidentiality |
| Denial of Service | Login service, API routes, and SQL database | Availability |
| Elevation of Privilege | Admin and inventory management functions | Authorization |

| Threat | STRIDE Category | Damage | Reproducibility | Exploitability | Affected Users | Discoverability | Total |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| SQL injection against product insert | Tampering / Information Disclosure | 8 | 6 | 5 | 7 | 6 | 32 |
| Credential brute force | Spoofing | 7 | 7 | 6 | 8 | 7 | 35 |
| Unauthorized product deletion | Elevation of Privilege / Tampering | 6 | 5 | 4 | 5 | 5 | 25 |
| Customer data disclosure | Information Disclosure | 8 | 5 | 5 | 8 | 6 | 32 |
| Order tampering | Tampering | 7 | 5 | 5 | 6 | 5 | 28 |
